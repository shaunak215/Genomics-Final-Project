...
def match_pattern(p, O, I, L, C, F, results_file):
    
    
    with open(results_file, 'w', encoding = 'utf-8') as o:
        ...
        while(1):
            if(curr_ind == 0):
                for s in  range(search_range_start, search_range_end+1):
                    ...
                    o.write("MATCH FOUND: \t" + " CHARACTER " + F[s] + " of RANK " + str(rank_c(s, F)))
...
def main(argv):
    ...
    if len (sys.argv) < 4 :
        print ("Usage: python pattern_matcher.py O.txt I.txt L.txt p path_to_output_file WG.dot(optional) output_graph.dot(optional)")
        sys.exit (1)
    ...
    pattern_match = match_pattern(p, O, I, L, C, F, results_file)
    
    if(pattern_match == -1):
        return -1
    
    all_visited_nodes, starting_edge, ending_edge = pattern_match
    
    
    if(len (sys.argv) > 5):
    
        input_wg = arguments[5]
        output_wg = arguments[6]
